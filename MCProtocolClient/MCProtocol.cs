...
namespace MCProtocol
{
    ...
    public class McProtocolTcp : McProtocolApp
    {
        ...
        {
            TcpClient c = _tcpClient;
            if (!c.Connected)
            {
                // install connection status monitor
                _connectionMonitor = new TcpClientConnectionMonitor(_tcpClient);
                _connectionMonitor.ConnectedChanged += OnConnectionChanged;
                _connectionMonitor.StartMonitoring();

                //
                // Keep Alive 기능 구현
                // c.f. : https://learn.microsoft.com/en-us/windows/win32/winsock/sio-keepalive-vals
                // WinSock에서 Keep-Alive 옵션을 켜면 OS가 자동으로 Keep-Alive 패킷을 보냄
                //
                var ka = new List<byte>(sizeof(uint) * 3);
                ka.AddRange(BitConverter.GetBytes(1u));         // Keep alive 사용할지 말지 : 0 이면 사용안함, 0이 아니면 사용함
                ka.AddRange(BitConverter.GetBytes(30_000u));    // Keep alive timeout : 첫 Keep-Alive 패킷을 보내기까지의 대기 시간 (30초)
                ka.AddRange(BitConverter.GetBytes(5_000u));     // Keep alive cycle : 30초가 지난 후, 5초마다 Keep alive 신호를 보낼 것임
                c.Client.IOControl(IOControlCode.KeepAliveValues, ka.ToArray(), null); // WinSock 에게 keep alive 사용한다고 설정함
        ...
        private TcpClientConnectionMonitor _connectionMonitor;
